...
if [[ -f "$SCRIPTS_DIR/tmux-confirm.sh" ]] && [[ -x "$SCRIPTS_DIR/tmux-confirm.sh" ]]; then
    pass "tmux-confirm.sh helper exists and is executable"
else
    fail "tmux-confirm.sh helper missing or not executable"
fi
...
for file in kill-session.sh rename-session.sh rename-window.sh timestamp.sh; do
