...
main() {
    while true; do
        ...
        action=$("$SCRIPT_DIR/list.sh" | fzf \
            ...
            --prompt=': ' \
            --border=rounded \
            --border-label=' j/k · ^d/^u · g/G · spc/⏎ sel · / srch · n new · e edit · x del · q/esc ' \
            ...
            --bind '/:enable-search+change-prompt(> )+unbind(j,k,g,G,ctrl-d,ctrl-u,q,n,e,x)' \
            --bind 'esc:transform:[[ $FZF_PROMPT == "> " ]] && echo "disable-search+clear-query+change-prompt(: )+rebind(j,k,g,G,ctrl-d,ctrl-u,q,n,e,x)" || echo "abort"' \
