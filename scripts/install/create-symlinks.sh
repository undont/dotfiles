...
if should_install "core"; then
    if [[ "$(uname)" == "Darwin" ]]; then
        ...
        # Symlink macOS location to XDG location (theme-switch generates to XDG)
        create_link "$HOME/.config/ghostty/config" "$HOME/Library/Application Support/com.mitchellh.ghostty/config"
    fi
fi
