...
-- CodeCompanion keybinds (disabled - use Copilot instead)
...
-- macOS-style keybindings in insert mode
...
require('lazy').setup({
  ...
  { -- CodeCompanion AI assistant (disabled - use GitHub Copilot instead)
...
vim.api.nvim_create_autocmd({ 'BufEnter', 'BufNew' }, {
  pattern = { '*.env', '*.env.*', '.env*', '*.secret*', '*credentials*', '*.pem', '*.key', '*.p12' },
  callback = function()
    vim.b.copilot_enabled = false
    vim.notify('Copilot disabled (sensitive file)', vim.log.levels.WARN)
  end,
})
