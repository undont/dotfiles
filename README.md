...
- **Diffs and PR review** via differ.nvim (my own plugin): local side-by-side diffs, file history, staging, GitHub PR review, and merge conflict resolution through one renderer, with `<leader>d*` diff launchers and `<leader>p*` PR launchers (thread actions are in-diff gestures: `ga` comment, `gp` reply, `gr` resolve); diffview stays for diff-by-ticket (`<leader>dT`) and Octo.nvim as a `:Octo` fallback, and gitsigns handles inline hunk decorations
...
- **Process list** (`` ` `` <kbd>Shift</kbd>+<kbd>P</kbd>) is one fzf switcher over everything you're watching: running commands (tests, builds, servers) as live ● rows with elapsed time, plus finished commands as ✓/✗ rows; jump to any, or press <kbd>x</kbd> to stop a running one or clear a finished one
