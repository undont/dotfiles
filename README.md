...
- **Handy aliases** — `gs` (git status), `gl` (git log), `gfp` (fetch + prune), `gpr` (prune local branches gone from remote), `grmc` (untrack files), `tattach` (attach or restore from backup), `cdb`/`cdf` (browser-style directory back/forward), `Opt+A` (directory history picker), `cl` (full terminal reset), `drs` (sync repo paths to gh-dash), `brewup`, `nvim-sync`, `mkcd`, and more — run `dot aliases` for the full list
