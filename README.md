...
- **Live agent state in the Claude switcher** (`` ` c ``): each instance shows what it's doing (● working, ◐ waiting for input, ○ idle, ✗ errored, ⚠ stuck) plus the age of its last activity, driven by Claude Code hooks writing per-pane state files; stuck detection cross-checks the pane title so long tool runs aren't flagged (see [docs/AGENT-HOOKS.md](docs/AGENT-HOOKS.md))
