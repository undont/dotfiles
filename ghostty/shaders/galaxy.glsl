// Galaxy shader for Ghostty
// Source: https://github.com/0xhckr/ghostty-shaders
//
// Animated nebula/galaxy background behind terminal text.
// Purple/blue cosmic clouds with volumetric rendering.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    const float baseBrightness = 0.0005;
    const float darkMatter = 0.2;
    const float distanceFading = 0.68;
    const float colorSaturation = 0.9;
    const float transverseMotion = 0.2;
    const float cloudOpacity = 0.30;
    ...
    accumulatedColor = mix(accumulatedColor, vec3(length(accumulatedColor) * 0.6), 0.35);
    ...
    backgroundColor.g *= 0.6;

    backgroundColor.b = 0.5 * mix(backgroundColor.g, backgroundColor.b, 0.6);
    ...
    if (terminalBrightness < brightnessThreshold) {
        fragColor = mix(terminalColor, vec4(foregroundColor.rgb + backgroundColor, 1.0), 0.10);
    } else {
