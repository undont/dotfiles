...
### Changed
- Nvim: `<leader>pe` (`Octo review resume`) now resumes at the file + cursor line you were editing if it's part of the PR — captures the current buffer's relative path + line before issuing the command, then a patched `Review:set_files_and_select_first` selects the matching `FileEntry` and restores the cursor on the right pane. Falls through to the default first-unviewed-file behaviour when the current buffer isn't in the PR. Mirrors `<leader>do`'s diffview cursor-restore flow but consults octo's review file list rather than local index diffs
...
### Fixed
- Nvim: `<leader>xx` (`vim.diagnostic.setqflist`) and `<leader>xX` (`vim.diagnostic.setloclist`) now pass explicit `Diagnostics: all` / `Diagnostics: buffer` titles, and `build.lua`'s `setup_auto_clear` extends to loclists in addition to the qflist. Previously these surfaces used the default `Diagnostics` title (no colon), so the `^(%w+):` predicate failed to match and resolved entries lingered until manually cleared. `Diagnostics` joins `Build`/`Sonar`/`Modified` in `AUTO_CLEAR_KINDS`; the loclist branch iterates non-qf windows and runs `:lclose` per-window when the list empties
