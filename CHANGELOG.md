...
- Claude switcher layout reworked to the proclist pattern: tab-delimited rows with the jump target hidden (`--with-nth=1`), a 60% preview that drops to a bottom split below 90 columns, and a state legend in the header, so rows have room for state and age. `tmux/tmux.conf.template`, `tmux/scripts/instances/claude.sh`
