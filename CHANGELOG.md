...
- Nvim: Sonar project scan — `<leader>ls` scans changed/untracked files (modified vs HEAD via `git diff --name-only --diff-filter=ACMR HEAD` plus `git ls-files --others --exclude-standard`), `<leader>lS` scans the whole project (`git ls-files -co --exclude-standard`, with a `vim.fs.find` fallback when not in a git repo). Both filter to sonarlint-supported extensions (.py/.ts/.tsx/.js/.jsx/.go/.cs/.c/.cpp/.h/.hpp/.php/.html/.css/.scss/.tf/.hcl/.yaml/.xml + Dockerfile), then hidden-load each file so the sonarlint LSP attaches and analyses. Analogue to JetBrains' "Analyze All Project Files" but scoped down for the changed-only mode (matches the SonarCloud PR-analysis use case). Detects "scan complete" via 2 s of quiet on `DiagnosticChanged` from the sonarlint client, with a 5 min hard timeout. Snapshots diagnostics into a `Sonar:`-titled quickfix and unloads only the buffers it created (pre-existing user buffers are left alone); qf items use `filename` not `bufnr` so navigation survives the buffer cleanup. Whole-project mode prompts for confirmation above 500 files; changed-mode is naturally bounded by the working tree. Fidget progress handle uses an `lsp_client.name = 'sonar-scan'` so it surfaces as a virtual LSP in the statusline
- Nvim: git-modified → quickfix workflow — `<leader>xm` and the `:GitModified` user command shell `git ls-files -m -o --exclude-standard`, `:args` the result so `:n`/`:N` step through them, `bufadd`+`bufload` so LSPs attach, then debounce on `DiagnosticChanged` (2 s quiet / 30 s hard timeout) and dump diagnostics scoped to those bufnrs into a `Modified: diagnostics`-titled quickfix. `<leader>xM` is the same flow with sonar-scale timings (5 s debounce / 5 min hard timeout) so sonarlint's java analyzers have time to publish — sonarlint auto-attaches via its `ft` lazy-load when buffers are loaded, so its diagnostics fold into the same qf without an explicit scan trigger. Sits next to `<leader>xx` (which stays a generic "all loaded buffers → qf" primitive — the modified-files variants are deliberately separate so the workflow keys don't surprise you when you're working through a fix in unrelated buffers)
- Nvim: `<leader>sm` telescope picker over `git_status` — modified + untracked files with diff previews, complementing `<leader>xm` (qf-driven) and `<leader>ls` (sonar scan)
- Shell: `lc` alias for `cl && lazycron` — completes the `j`/`lg`/`ld` family of clear-then-launch TUI shortcuts

### Changed
- Nvim: `build.lua`'s qf auto-clear (drops items as their underlying diagnostics resolve) now also fires on `Sonar:`-titled lists — the prune logic is identical, only the "all clear" notification text varies (`Build errors resolved` vs `Sonar issues resolved`)
...
- Nvim: `grr`/`gri`/`grd`/`grt` now surface a warning notification when the LSP returns no results (`No references found`, `No implementations found`, etc). The previous `lsp_dedup` path relied on `vim.lsp.buf.<method>`'s `on_list` callback, but Neovim's built-in handlers short-circuit on empty results before `on_list` runs — the location methods (`gri`/`grd`/`grt`) failed silently and `grr` fell back to the stock `No references found` message. The helper now drives the LSP request directly via `vim.lsp.buf_request_all`, so the dedup/empty-state branch owns the full response lifecycle; single-result jump-to and multi-result telescope quickfix behaviour is unchanged. `grr` also flips `includeDeclaration` to `false` (Neovim default is `true`) so pressing it on a truly-unused symbol fires the warning instead of jumping to the declaration itself — the common intent is "show me the callers", not "confirm this symbol exists"
- Nvim: scan helpers unified into `core/scan_runner.lua` — extracted the duplicated debounce / `DiagnosticChanged` / qf-collection state machine that previously lived independently in `sonarlint.lua` and `core/lists.lua` (~110 LOC of duplication removed). User-visible behaviour change: the two scan flows now share a single global singleton, so triggering `<leader>xm` while `<leader>ls` is running (or vice versa) reports `A scan is already running` instead of allowing both to write to the qf concurrently. The previous concurrent path had cross-pollination potential — items from a still-running sonar scan could be overwritten by the modified-scan finalise — so mutual exclusion matches intent
...
- Tmux: launcher picker and wizard now strip shell-escape backslashes from pasted directory paths — typing/pasting `~/Library/Mobile\ Documents/iCloud\~md\~obsidian/...` into the fzf directory prompt was passed through verbatim, so the `[[ -d "$dir" ]]` check failed (fzf input is literal text, not a shell-parsed word) and the wizard baked the backslashes into the generated launcher's double-quoted `PROJECT_DIR` where `\ ` and `\~` aren't escape sequences. Fix unescapes `\ ` → space and `\~` → `~` at three spots: `run.sh`'s parameterised directory picker, and `new.sh`'s project-dir (step 2) and worktrees-dir (step 3 sub-3) pickers. Launchers generated before this fix need to be edited by hand (drop the backslashes from `PROJECT_DIR`)
- Nvim: `lsp_dedup` (`grr`/`gri`/`grd`/`grt`) now builds the `textDocument/position` payload per-client instead of computing it once from `clients[1].offset_encoding`. `vim.lsp.buf_request_all` accepts a callable form for `params` so each LSP gets a request encoded in its own negotiated `offset_encoding` — the previous static-table call broadcast the first client's encoding to every client and could land on the wrong byte for non-first clients in mixed-encoding setups (e.g. a utf-8 rust-analyzer attached alongside a utf-16 lua_ls) when the cursor was on a line with multibyte characters. The response side already used the per-client encoding correctly via `client.offset_encoding`; this aligns the request side with the same contract
- Nvim: sonarlint connected-mode `notify_connection_result` now `tostring`-coerces and clamps `params.reason` to 200 chars before splicing it into the `Cannot connect to ...` notification — the field is server-supplied, so a malformed/long/multi-line value (or non-string type) shouldn't be allowed to bloat the notify log
- Nvim: `<leader>xm` modified-scan now wraps `vim.fn.bufload` in `pcall` (matches the existing pattern in `sonarlint.lua`'s scan loop) so a single un-loadable file (e.g. permissions `000`, broken symlink) skips silently instead of throwing `E484: Can't open file` and aborting the entire scan. The `bufnr` from `bufadd` stays valid even when `bufload` fails — diagnostics simply won't populate for that file, which is the desired graceful degradation

### Refactor
- Nvim: `build.lua`'s qf auto-clear `Build`/`Sonar` predicate collapsed into an `AUTO_CLEAR_KINDS` kind→label table — adding a third kind (e.g. `Modified` for the `<leader>xm` qf) becomes a one-line addition instead of editing both the predicate and the notification ternary. No behaviour change
- Tmux: 3 verbatim copies of the shell-escape backslash-strip block (`run.sh`'s parameterised picker, `new.sh`'s project-dir step 2 and worktrees-dir step 3 sub-3) extracted into an `unescape_paste` helper local to each launcher. No behaviour change — the strip semantics (`\ ` → space, `\~` → `~`) are identical to the inline version
